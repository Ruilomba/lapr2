...
public class Data implements Comparable<Data>, Serializable {
    ...
    /**
     * Constrói uma instância da Data recebendo uma string com a data.
     * Separador "/".
     * @param data data
     */
     ...
     * Separador "/".
     * @param data data
     * @throws DataInvalidaException data inválida
     */
    public Data(String data)  {
        String[] parts = data.split("/");
        int l_ano = Integer.parseInt(parts[2]);
        int l_mes = Integer.parseInt(parts[1]);
        int l_dia = Integer.parseInt(parts[0]);
        setData(l_ano, l_mes, l_dia);
    }
    ...
    public final void setData(int ano, int mes, int dia) {
        if (mes < 1 || mes > 12) {
            throw new DataInvalidaException("Month " + mes + " is invalid!!");
        }
        if (dia < 1 || (mes == 2 && Data.isAnoBissexto(ano) ? 
                                    dia > 29 : 
                                    dia > Data.diasPorMes[mes]))  {
            throw new DataInvalidaException("Day " + ano + "/" + mes + "/" + dia
                    + " is invalid!!");
        }
        this.ano = ano;         
        this.mes = mes;       
